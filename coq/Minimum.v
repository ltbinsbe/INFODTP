...
(* proving that the invariant holds when 'foldl1 join' is called, assuming that
 * 'fold_right step []' produces a strictly increasing list 
 *)
 ...
 * preserves the fact that the left-most pair is still a local minimum pair. assuming that
 * foldl_left acts as it should 
 *)
...
Proof.
    ...
    (* ass 1 *)
    simpl. replace (max (ht t) (ht u) + 1) with (S (max (ht t) (ht u))).
    apply lt_le_S. unfold max. remember (nat_compare (ht t) (ht u)) as R.
    destruct R.
        inversion Sinc. inversion H0. inversion H4.
        apply lt_trans with (m := ht u).
        assumption. assumption.
        apply lt_trans with (m := ht u). assumption.
        inversion H6. assumption.
        inversion Sinc. inversion H0. inversion H4. assumption.
        inversion H6. assumption.
        inversion Sinc. inversion H0. inversion H4.
        apply lt_trans with (m := ht u). assumption. assumption.
        apply lt_trans with (m := ht u). assumption. inversion H6. assumption.

    (* replace *)
    symmetry. apply NPeano.Nat.add_1_r.
    induction ts.
        apply lmp_pair.

        (* step *)
        remember (nat_compare (ht (join t u)) (ht v)) as R1.
        destruct R1.

        (* R1 = Eq *)
        apply lmp_threer. right. split. apply eq_ge. assumption.
        inversion Sinc. inversion H1. inversion H5. inversion H7. inversion H11.
        replace (ht (join t u)) with (ht v). assumption.
        symmetry. apply nat_compare_eq. symmetry. assumption.

        replace (ht (join t u)) with (ht v). inversion H13. assumption.
        symmetry. apply nat_compare_eq. symmetry. assumption.

        (* R1 = LT *)
        apply lmp_threer. left. split. apply nat_compare_lt. symmetry. assumption.
        inversion Sinc. inversion H1. inversion H5. inversion H7. inversion H11. assumption.
        inversion H13. assumption.

        (* R1 = GT *)
        apply lmp_threer. contradict H.
        apply lt_not_le. apply nat_compare_gt. symmetry. assumption.
Qed.

(* Now given that Lemma1 proves that joining a lmp in a list of trees preserves the fact that we
 * are on the way of building a tree of minimum height, foldl1 creates a tree of minimum height 
 *)
...
Theorem foldl1_is_min:
    forall
        (l1 l2 : list tree) (P : l1 <> [])
        (H : l1 = (fold_right (fun  (a : tree) (xs : list tree) => step a xs) [] l2)),
